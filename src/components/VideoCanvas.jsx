...
import classes from "./VideoCanvas.module.css";
import VisualizeAudio from "./VisualizeAudio";
import { formatDuration } from "../utility/commonFunction";

const VideoCanvas = ({ videoFile, fileInfo }) => {
  ...
  useEffect(() => {
    ...
    function readyToPlayVideo() {
      ...
      document.getElementById("playPause").textContent = "click to play pause";
    }
  ...
  return (
    <>
      <div className={classes["container-canvas"]}>
        <div>
          <canvas id="myCanvas" width="532" height="300" ref={canvasRef} className={classes.canvas}></canvas>
          <h3 className={classes.h3}>
            <div id="playPause">Loading content.</div>
          </h3>
          <br />
          <canvas id="myCanvas2" width="532" height="100" ref={canvasRefAudio} className={classes.canvas}></canvas>
        </div>
        {metaData && (
